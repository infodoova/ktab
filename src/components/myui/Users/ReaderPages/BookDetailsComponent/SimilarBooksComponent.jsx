      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-6">
        {books.map((simBook) => (
          <div
            key={simBook.id}
            onClick={() =>
              navigate(`/Screens/dashboard/ReaderPages/BookDetails/${simBook.id}`, { state: { book: simBook } })
            }
          ...
          >
            ...
            {/* Details */}
            <div className="mt-3">
               <h3 className="text-sm font-bold text-[#3E2723] line-clamp-1 group-hover:text-[#606C38] transition-colors">{simBook.title}</h3>
               <div className="flex items-center gap-2 text-xs text-[#5D4037] mt-1 font-medium">
                 <span className="border border-[#D7CCC8] px-1 rounded text-[10px]">موصى به</span>
                 <span className="text-[#606C38]">%95 تطابق</span>
               </div>
            </div>
          </div>
        ))}
